...
function cdpg_service_page()							
{
    ...
    if (isset($_POST['cdpg_generate'])) {
        cdpg_generate_service_page();
    }

   echo '<h1>Генератор Услуг</h1>';

// Вкладки
echo '<div class="cdpg-tabs">';
echo '<div class="cdpg-tab cdpg-tab-active" id="cdpg-tab-generation">Генерация услуг</div>';
...
echo '<div class="cdpg-inner-tab-content-item cdpg-inner-tab-content-ee cdpg-inner-tab-content-active" id="cdpg-inner-tab-ee-content">';
    echo '<h3>Описание услуги на Эстонском языке</h3>';
...
echo '<option value="Kiirremont">Kiirremont</option>';
echo '<option value="Diagnostika">Diagnostika</option>';
echo '<option value="Ekraani vahetus HQ">Ekraani vahetus HQ</option>';
echo '<option value="Ekraani vahetus originaal">Ekraani vahetus originaal</option>';
echo '<option value="Kaitsekile">Kaitsekile</option>';
echo '<option value="Kaitseümbris">Kaitseümbris</option>';
...
echo '</div>';
    ...
    acf_form(array(
        'post_id' => 8402,
        'field_groups' => array('group_6364d74731b42'), // Эстонская группа полей
        'form' => false,
        'field_el' => 'div',
        'html_before_fields' => '',
        'html_after_fields' => '',
...
echo '<div class="cdpg-inner-tab-content-item cdpg-inner-tab-content-ru" id="cdpg-inner-tab-ru-content">';
    echo '<h3>Описание услуги на Русском языке</h3>';
...
echo '<option value="Срочный ремонт">Срочный ремонт</option>';
echo '<option value="Диагностика">Диагностика</option>';
echo '<option value="Замена дисплея оригинал">Замена дисплея оригинал</option>';
echo '<option value="Замена дисплея HQ">Замена дисплея HQ</option>';
echo '<option value="Защитная пленка на">Защитная пленка на</option>';
echo '<option value="Защитное стекло на">Защитное стекло на</option>';
echo '<option value="Защитный чехол на">Защитный чехол на</option>';
...
echo '</div>';
 ...
 // Русская форма
    acf_form(array(
        'post_id' => 8405,
        'field_groups' => array('group_645b95efc10d1'), // Русская группа полей
        'form' => false,
        'field_el' => 'div',
        'html_before_fields' => '',
        'html_after_fields' => '',
...
// Содержимое для "Логи"
    $logs = file(plugin_dir_path(__FILE__) . 'logs-services.txt'); // Чтение логов из файла
    if ($logs) {
        echo '<table>';
...
function cdpg_generate_service_page()
{
...
$page_title_ru = $service_ru . ' ' . $device_model_ru . ' ' . $additional_field_ru;
    ...
    if ($page_id_ee && $page_id_ru) {
        ...
        $fields_ee = acf_get_fields('group_6364d74731b42');
        if ($fields_ee) {
        ...
        $fields_ru = acf_get_fields('group_645b95efc10d1'); // Русская группа полей
        if ($fields_ru) {
